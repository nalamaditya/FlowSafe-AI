    ...
    <section className="bg-white rounded-2xl border border-slate-200 p-5 md:p-6 shadow-sm space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-slate-100">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-bold uppercase tracking-wider text-purple-600 bg-purple-50 px-2.5 py-1 rounded-full border border-purple-200">
              STRESS TEST
            </span>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Simulate sudden crowd surges or unexpected influx for <strong className="text-slate-800">{simulation.location.name}</strong> at <strong className="text-blue-600">{simulation.targetTimeFormatted}</strong>.
          </p>
        </div>
      </div>
      ...
      <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-black font-mono text-purple-600 bg-purple-50 border border-purple-200 px-3 py-1 rounded-lg">
        ...
        <div className="flex justify-between text-[10px] font-mono text-slate-400">
          ...
          <span>+100 (Medium Surge)</span>
          <span>+200 (Heavy Surge)</span>
          <span>+300 (Extreme Influx)</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="p-4 rounded-xl border border-slate-200 bg-white space-y-2.5">
          <div className="flex items-center justify-between pb-2 border-b border-slate-100">
            <span className="text-xs font-bold text-slate-600 uppercase">Original Baseline</span>
            <span className={`text-[10px] px-2 py-0.5 rounded-full border ${simulation.original.status.badgeClass}`}>
          ...
          <div className="flex justify-between text-xs font-mono">
            ...
            <strong className="text-slate-900 text-sm">{simulation.original.crowd}</strong>
          </div>

          <div className="flex justify-between text-xs font-mono">
            <span className="text-slate-500 font-sans">Occupancy:</span>
            <strong className="text-slate-900 text-sm">{simulation.original.occupancy}%</strong>
          </div>

          <div className="flex justify-between text-xs font-mono">
            <span className="text-slate-500 font-sans">Estimated Wait:</span>
            <strong className="text-slate-900 text-sm">{simulation.original.wait} mins</strong>
          </div>
        </div>
        ...
        <div className={`p-4 rounded-xl border space-y-2.5 ${
          ...
          <div className="flex items-center justify-between pb-2 border-b border-slate-200/60">
            <span className="text-xs font-bold text-slate-900 uppercase">
              What-If Scenario (+{additionalVisitors})
            </span>
            <span className={`text-[10px] px-2 py-0.5 rounded-full border ${simulation.scenario.status.badgeClass}`}>
          ...
          <div className="flex justify-between text-xs font-mono">
            ...
            <strong className="text-purple-700 text-sm font-black">{simulation.scenario.crowd}</strong>
          </div>

          <div className="flex justify-between text-xs font-mono">
            <span className="text-slate-600 font-sans">Scenario Occupancy:</span>
            <strong className={simulation.scenario.occupancy >= 85 ? 'text-red-600 text-sm font-black' : 'text-slate-900 text-sm font-black'}>
      ...
      <div className="p-4 rounded-xl bg-purple-50/40 border border-purple-200 text-xs font-medium text-slate-800 space-y-1">
