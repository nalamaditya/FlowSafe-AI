    ...
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-sm space-y-3 sm:space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
          ...
          <span className="text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-200 font-mono self-start sm:self-auto shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 pt-1 text-xs">
          <div>
            <label className="font-bold text-slate-700 block mb-1 text-xs">Target Zone to Test:</label>
            <select
              value={selectedLocationId}
              onChange={(e) => setSelectedLocationId(e.target.value)}
              className="w-full px-3 py-2 rounded-lg bg-slate-50 border border-slate-300 font-semibold text-slate-900 focus:outline-none focus:border-purple-600 cursor-pointer text-xs"
            >
          ...
          <div>
            <label className="font-bold text-slate-700 block mb-1 text-xs">Target Time Scenario:</label>
            <select
